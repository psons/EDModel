//
//  Objective.swift
//  EffortDomain
//
//  Created by Paul Sons on 3/11/23.
//

import Foundation

public let defaultMaxTasks = 1

// Objective replaces Story in the class model.
public class Objective: Codable, CustomStringConvertible {
    public static let defaultTslot = 0
    public static let invalidTslot = -1

    var name: String
    var maxTasks = defaultMaxTasks
    let oid: String
    var tasks: [Task] = []
    public var description: String {
        let heading = "{Objective} |maxTasks:\(maxTasks)|name: \(name)|oid: \(oid)|\n"
        var tasksAsStr = ""
        for task in tasks {
            tasksAsStr += "\t\t\t\(task)\n"
        }
        return heading + tasksAsStr
    }

    
    public init(name: String, maxTasks: Int = defaultMaxTasks) {
        self.name = name
        self.maxTasks = maxTasks
        self.oid = "Need to implement oid in init"
    }

    public func addTask(task: Task) {
        self.tasks.append(task)
    }

}
